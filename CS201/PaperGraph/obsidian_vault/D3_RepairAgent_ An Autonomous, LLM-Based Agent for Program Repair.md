...
arxiv_id: "2403.17134"
confidence: verified
source: "arXiv:2403.17134"
node_type: paper
---

# RepairAgent: An Autonomous, LLM-Based Agent for Program Repair

**Domain**: [[domain_D|Automated Code Repair]] | **Year**: 2024 | **Confidence**: [x] verified
...
## Keywords
- [[kw_RepairAgent|RepairAgent]]
- [[kw_autonomous repair|autonomous repair]]
- [[kw_LLM agent|LLM agent]]
- [[kw_program repair|program repair]]
- [[kw_tool-augmented|tool-augmented]]
...
introduces RepairAgent, the first work to address the program repair challenge through an autonomous agent based
...
agent for program repair, paving the way for future agentbased techniques in software engineering.

I. INTRODUCTION

arXiv:2403.17134v2  [cs.SE]  28 Oct 2024
...
the
first
autonomous, LLM-based agent for automated program
...
Software bugs lead to system failures, security vulnerabilities, and compromised user experience. Fixing
bugs is a critical task in software development, but
if done manually, demands considerable time and effort. Automated program repair (APR) promises to dramatically reduce this effort by addressing the critical
...
constraints [7], [8], [9], and various machine learningbased approaches [10], [11], [12], [13], [14], [15], [16].
...
LLMs have demonstrated remarkable abilities in achieving human-level performance for various tasks [26]. A
...
tools that enable the LLM to interact with its environment [27], [28]. In the context of software engineering,
and automated repair in particular, the tools could be
tools usually used by developers, e.g., as part of an integrated development environment (IDE). The basic idea is
...
RepairAgent is capable of fixing complex bugs. Compared to state-of-the-art techniques [19], [21], RepairAgent successfully fixes 39 bugs not fixed by prior work.
...
Figure 1 gives an overview of the RepairAgent approach, which consists of three components: an LLM
agent (left), a set of tools (right), and a middleware that
orchestrates the communication between the two (middle). Given a bug to fix, the middleware initializes the
LLM agent with a prompt that contains task information
and instructions on how to perform it by using the provided tools (arrow 1). The LLM responds by suggesting
...
1) Role:
This section of the prompt defines the
...
• Locate the bug: Execute tests and use fault localization techniques to pinpoint the bug’s location. Skip
...
base, which serves as the basis for deciding which commands to invoke next. To make this information available
...
for the agent, allowing it to recall information from previous cycles. The gathered information is structured into
different subsections, where each subsection contains the
outputs produced by a specific tool.
7) Specification of Output Format: Given the dynamic prompt, the LLM agent provides one response
per cycle. To enable the middleware to parse the response, we specify the expected output format (Figure 3).
...
agent. The model expresses the need to collect more information to understand the bug and suggests a command
...
extract method tool, the agent can retrieve the implementation(s) of methods that match a given method name
...
enables the agent to locate instances of particular keywords within the entire code base. For example, the
agent can use this tool to find occurrences of variables, methods, and classes. Given a set of keywords,
...
quickSort, arrayQuickSort, and other related variations. The output of the tool is a nested dictionary,
...
In addition to searching for existing code, RepairAgent offers a tool that generates new code by invoking
...
bugs. Given the code preceding a method and the signature of the method, the generate method body tool asks
...
same LLM for both the repair agent and as the codegenerating LLM of this tool. The tool limits the given
...
tool cleans the output of the test runner, e.g., by removing entries of the stack trace that are outside of
...
write fix
Apply a patch to the code base and execute the test suite of the project. Changes are reverted automatically if tests
...
4) Control: The final set of tools do not directly correspond to a tool a human developer may use, but rather
...
1) Parsing and Refining LLM Output: At the beginning of each cycle, the middleware queries the LLM
...
In addition to rectifying minor mistakes in the response, the middleware also checks for repeated invocations of the same tool with the same arguments. If
...
To prevent tool executions to interfere with the host environment or RepairAgent itself, the middleware executes
...
RepairAgent on bugs from a newer dataset, GitBugJava [25]. All bugs in this dataset were discovered and
...
b) Baselines: We compare with three existing repair techniques: ChatRepair [19], ITER [21], and SelfAPR [38]. ChatRepair and ITER are two very recent approaches and have been shown to be the current state of
...
command executions for enhanced reliability and reproducibility. RepairAgent is built on top of the AutoGPT
...
1) Overall Results:
Table III summarizes the effectiveness of RepairAgent in fixing the 835 bugs in
...
Project
Bugs Plausible Correct ChatRepair ITER SelfAPR
Chart
26
14
11
...
can generalize to code bases of multiple domains. Furthermore, RepairAgent creates fixes for bugs of different
...
the generalization capabilities of RepairAgent, we evaluate the approach on GitBug-Java, with results shown in
...
Project
Plausible Correct Cost($) SL ML MF
No search tools
14
11
28 11
...
for GitBug-Java. We conclude that RepairAgent generalizes well to new projects and bugs, and is not strongly
...
To better understand the impact of different components and configurations of RepairAgent, we perform the
...
configurations), out of which the full RepairAgent approach fixes 21 bugs. We report the number of plausible
and correct patches, costs in US dollars, and a breakdown of correct fixes into single-line (SL), multi-line
...
We measure three kinds of costs imposed by RepairAgent: (i) Time taken to fix a bug. (ii) The number of
tokens consumed by queries to the LLM, which is relevant both for commercial models, such as the GPT-3.5
...
generate fixes. Approaches include neural machine translation models that map buggy code into fixed code [11],
[12], [13], [14], models that predict tree transformations [15], [16], neural architectures for specific kinds of
...
for bugs that only require simple modifications. A possible remedy could be to initially limit the complexity of
...
While RepairAgent shows promising results, we acknowledge several potential threats to validity and inherent limitations: (i) Data leakage: GPT-3.5 may have seen
...
a) Non-learning-based
program
repair:
Automated program repair [1] has received significant attention. Some approaches address it as a search problem
...
autonomously plan and perform complex tasks is relatively new and has been applied to tasks outside of
...
of this paper was made publicly available, other LLMbased agents for software engineering tasks have been
...
Our key contribution in applying these ideas to a software engineering task is to define tools that are useful
...
Language Models (LLMs). Through extensive experimentation, we validate the effectiveness and potential
...
[21] H. Ye and M. Monperrus, “Iter: Iterative neural repair for multilocation patches,” in ICSE, 2024.
...
[25] A. Silva, N. Saavedra, and M. Monperrus, “Gitbug-java: A reproducible benchmark of recent java bugs,” in Proceedings of the
...
of artificial general intelligence: Early experiments with GPT4,” CoRR, vol. abs/2303.12712, 2023. [Online]. Available:
...
[31] R.-M. Karampatsis and C. Sutton, “How often do singlestatement bugs occur?” Jun. 2020. [Online]. Available: http:
//dx.doi.org/10.1145/3379597.3387491
[32] J. Wei, X. Wang, D. Schuurmans, M. Bosma, F. Xia, E. Chi, Q. V.
Le, D. Zhou et al., “Chain-of-thought prompting elicits reasoning in large language models,” Advances in neural information
...
[4] D. Kim, J. Nam, J. Song, and S. Kim, “Automatic patch generation learned from human-written patches.” in International
...
“Semfix: program repair via semantic analysis,” in 35th International Conference on Software Engineering, ICSE ’13, San
...
using ensemble for program repair,” in ISSTA.
ACM, 2020, pp.
...
programs with semantic code search (t),” in ASE.
IEEE, 2015,
pp. 295–306.
...
2019.
AAAI Press, 2019, pp. 930–937. [Online]. Available:
...
bottlenecks in concurrent programs,” Empirical Software Engineering (EMSE), pp. 1–38, 2017.
...
May 2021.
IEEE, 2021, pp. 359–371. [Online]. Available:
...
Eds.
AAAI Press, 2023, pp. 5131–5140. [Online]. Available:
...
[59] D. Shrivastava, H. Larochelle, and D. Tarlow, “Repositorylevel prompt generation for large language models of code,” in
...
K. Narasimhan, and O. Press, “Swe-agent: Agent-computer interfaces enable automated software engineering,” 2024.
